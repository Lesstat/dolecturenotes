...
- This problem is incredibly hard; it is not only NP-hard to find the
  optimum solution; even worse one can show, that no algorithm can
  guarantee a solution whose cost is less than a log n factor more
  than the optimum. (based on similar complexity theoretical
  assumption $P \not= NP$)
** Moddeling SetCover as (Integer) Linear Programm
...
- Figure Tut 4_1
- an initial v-shape of $P_A$ is defined by $c^Tx \leq  + d-1 linear
